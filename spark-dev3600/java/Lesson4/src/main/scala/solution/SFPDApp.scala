...
object SFPDApp {
  def main(args: Array[String]) {
    ...
    val sfpdFile = "/headless/Desktop/next-level-python-big-data/spark-dev3600/data/sfpd.csv"
    ...
    val sfpdDS = spark.read.format("csv").option("inferSchema", true).load("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/sfpd.csv").toDF("incidentnum", "category", "description", "dayofweek", "date", "time", "pddistrict", "resolution", "address", "X", "Y", "pdid").cache
