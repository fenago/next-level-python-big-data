...
// Load the data
val ratingText = sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/ratings.dat")
...
val moviesDF= sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/movies.dat").map(parseMovie).toDF()  
val usersDF = sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/users.dat").map(parseUser).toDF() 
