...
val sfpdRDD = sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/sfpd.csv").map(line => line.split(","))
...
val catAdd = sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/J_AddCat.csv").map(x => x.split(",")).map(x => (x(1), x(0)))
val distAdd = sc.textFile("/headless/Desktop/next-level-python-big-data/spark-dev3600/data/J_AddDist.csv").map(x => x.split(",")).map(x => (x(1), x(0)))
